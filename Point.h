...
public:
    static std::shared_ptr<Point> create(std::shared_ptr<SGL::Geometry> geometry, std::shared_ptr<SGL::ShaderMaterial> material);
    void set_size(float r);
    void set_smooth(bool smooth);
protected:
    Point(std::shared_ptr<SGL::Geometry> & geometry, std::shared_ptr<SGL::ShaderMaterial>& material) {
        m_material = material;
        m_geometry = geometry;
        set_size(1.f);
        set_smooth(true);
